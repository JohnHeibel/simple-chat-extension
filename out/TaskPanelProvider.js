...
class TaskPanelProvider {
    constructor(extensionUri) {
        this._onTaskChanged = new vscode.EventEmitter();
        this.onTaskChanged = this._onTaskChanged.event;
    ...
    async _getCurrentCode() {
        ...
        const codePath = path.join(workspaceFolders[0].uri.fsPath, trialConfig.task_dir, trialConfig.module_filename);
    ...
    async _handleLoadTask() {
        ...
        try {
            ...
            const response = await axios_1.default.get(`${backendUrl}/api/tasks/current/${trialConfig.participant_id}`);
            this._view?.webview.postMessage({
                command: 'taskLoaded',
                data: response.data,
            });
    ...
    async _handleRunTests() {
        ...
        try {
            const backendUrl = this._getBackendUrl();
            const response = await axios_1.default.post(`${backendUrl}/api/tasks/run-tests/${trialConfig.participant_id}`, { code });
            this._view?.webview.postMessage({
                command: 'testResults',
                data: response.data,
            });
    ...
    async _handleSubmitCode() {
        ...
        try {
            const backendUrl = this._getBackendUrl();
            const response = await axios_1.default.post(`${backendUrl}/api/tasks/submit/${trialConfig.participant_id}`, { code });
...
TaskPanelProvider.viewType = 'trialTaskView';
